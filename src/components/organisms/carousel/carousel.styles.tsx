...
const productBackground = ({ index }: { index: number}) => {
	if (!index) return `#ECDCC6`;
	if (index % 2 === 0) return `#ECDCC6`;
	return `#c6e3ec`;
};

export const StyledCarousel = styled.section<{ type?: string;}>`
...
export const SyledCarouselItem = styled.div<{index: number; type: string;}>`
	height: 100%;
